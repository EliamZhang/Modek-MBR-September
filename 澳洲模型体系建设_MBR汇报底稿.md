...
> **用途**：面向管理层 2026 年 9 月 MBR 汇报的内容底稿，按页面主题组织，可直接拆页制作 PPT。
> **资料依据**：`res/` 目录素材 + 模型创新申请材料、2026H2 OKR、BS-CAT 进展材料（详见文末《数据依据与待确认事项》）。
> **标记说明**：【待补充】= 材料未给出的数字，不得编造；【待确认】= 口径或状态需与相关团队核对后使用。

---

## 开场｜汇报主线

**核心观点**：澳洲业务模型体系正在完成从「风险单一判断」到「风险 × 价值 × 意愿三维经营」的升级，底层由自主可控的银行流水能力支撑，目标形成数据能力、模型能力、业务决策能力的一体化体系。

**主线四段**：

1. **为什么现在**：获客贵、转化长、监管严——澳洲经营的三个结构性约束，决定了决策必须从经验走向量化分层；
2. **判断什么**：从「能不能借」（风险）扩展到「值不值得经营」（价值）、「想不想借」（意愿），三个模型联合回答，指导分层、定价、额度与投放；
3. **靠什么支撑**：自研银行流水体系（BS-CAT）——澳洲模型最重要的数据源，从依赖 illion 走向自主可控，数据更细、更稳、迭代更快；
4. **走向哪里**：数据 → 能力 → 模型 → 决策 → 业务应用的完整闭环，投放、运营、风控、定价共用一套分层标准。

```mermaid
flowchart LR
    A[经营约束<br/>获客贵 · 转化长 · 监管严] --> B[量化分层经营<br/>风险 × 价值 × 意愿]
    B --> C[模型体系<br/>风险模型 · 价值模型 · 意愿模型]
    C --> D[业务决策<br/>投放 · 运营 · 风控 · 定价]
    E[自研银行流水能力<br/>BS-CAT 分类 + S-CALC 测算] --> C
    F[知识库 + AI Agent<br/>运维与产能] -.-> E
    F -.-> C
```
...
**核心观点**：澳洲信贷业务的三个结构性约束——获客贵、转化长、监管严——决定了「粗放经营」不可持续，量化分层经营是规模与风险平衡的唯一路径。

**关键信息**：

- **获客贵**：金融类是 2026 年获客成本最高的品类之一；投放粗放模式效率极低——召回触达 13.6k 用户仅带来约 30 个 Leads，「找一批人 → 发触达」缺乏「谁值得买、花多少钱买」的预测能力；
- **转化长**：信贷转化链路长（曝光 → 下载 → 注册 → 申请 → 授信 → 放款），iOS 归因不完整，投放效果长期无法稳定回答「这笔 Loan 是哪次投放带来的」；
- **监管严**：澳洲 Responsible Lending 监管（NCCP Act 2009 / ASIC）要求核实客户收入、必要支出与现有负债，每一笔授信决策必须有据可依；
- **经营目标**：2026H2 放款规模达成（OKR O1）、公司 O2-KR1「NON SAAC 占比全年不低于 40%、风险表现 7.1% 目标」。

**建议展示**：三约束卡片 + OKR 目标条，一页立题。

### P2｜当前处于什么阶段

**核心观点**：澳洲模型已完成「从无到有」的阶段跨越——核心模型均已上线并验证有效，当前正处于「体系化落地」阶段：效果已验证，正在打通业务应用链路与数据供给链路。

**关键信息**：

- **已完成并验证**：新客价值模型 1.0（OOT AUC 0.729）、老客风险模型 V2（OOT AUC 0.709，较 V1.1 提升 2.84%）、复贷响应模型（AUC 0.703）、WageGo 新客审批风险主模型（2026-06 版本生产运行）、银行流水分类模型 BS-CAT（8 月完成 preview 部署并开放接口）；
- **8 月里程碑**：负债类、收入类知识库初始化完成，Liability 效果报告发布；
- **当前推进中**：消费类知识库建设、新 S-CALC 开发、新 IDA 调用链路开发、两个运维 Agent 开发；
- **9 月目标**：正式版模型发布、分类数据落库、IDP/IDA 链路调通、新旧链路 AB 测试，空跑验证后具备切量条件（下游正在评估切量时间节点）。

**建议展示**：时间线图（8/9 → 8/11 → 8/21 → 8/31 → 9/15 → 切量）。

### P3｜已完成什么：模型版图覆盖客户全生命周期

**核心观点**：已形成覆盖「新客 → 复贷 → 老客」全生命周期的模型版图，风险、价值、意愿三类判断并行，全部经过跨时间样本（OOT）验证。
...
| 新客经营 | 新客价值模型 1.0（LightGBM，17 特征） | AUC 0.729 / KS 0.347，Top10% 目标率 63.00% vs 底部 7.30% | 已评估通过，支持授信 / 定价 / 审批 / 分层 |
...
| 底层数据 | 银行流水分类模型 BS-CAT（9 引擎）     | 对 illion 已识别覆盖 98.5%，净增识别 317 条              | preview 已部署，8/31 发布正式版           |

**建议展示**：客户生命周期横轴（新客 → 复贷 → 老客）+ 模型卡片矩阵，每个模型带一个关键数字。

### P4｜模型之间是什么关系：三维分层一体化体系

**核心观点**：模型体系从「只看风险」演进为三维判断——风险回答「能不能借」、意愿回答「想不想借」、价值回答「值不值得经营」，三者联合形成统一分层标准，向上支撑投放、运营、定价三类决策。

**关键信息**：

- **三层结构**：数据层（银行流水 + 平台行为 + 征信/替代数据）→ 模型层（风险 / 价值 / 意愿三类模型）→ 决策层（人群分层、额度、定价、审批、投放出价）；
- **统一分层标准**（Risk × Intent × Value，落地投放与运营）：低风险 × 高意愿 × 高价值 = 优质人群（保量提价、核心转化目标）；高风险 × 高意愿 = 风险控制人群（不出价或素材劝退）；低风险 × 低意愿 × 高价值 = 激活人群；中风险 × 高意愿 × 高价值 = 定价优化人群；
...
**核心观点**：H2 以「银行流水切量、分层体系落地、AI 运维试点」三条主线推进，与投放、运营团队以项目清单方式协同，P0 事项聚焦「找谁买、花多少钱买、买来值不值、买的人好不好」。
...
- **银行流水侧**：正式版发布、分类数据落库、新旧 IDP 链路 AB 测试、S-CALC 与 LMS 改造（8/31—9/15 里程碑）；
...
- **对组织效率**：银行流水体系以「1 人 + 2 个 AI Agent」实现运维闭环，AI 辅助分析、监控、诊断，释放模型团队产能。
...
### P1｜管理层最关心的问题：额度与价值，谁是鸡谁是蛋

**核心观点**：「价值模型是不是在解释历史额度策略」的质疑已被正面回应——同额度分层验证证明，价值信号存在于额度之外。

**关键信息**：

- **问题实质**：客户价值高，是因为我们给了更高额度导致利息收入更多；还是客户本身价值高，所以应该给更高额度？模型到底是在发现价值，还是在复述历史策略？
- **直接回答（不回避）**：时间顺序上额度在先、价值在后，这一点承认；但额度能否转化为价值取决于客户自身资质——同样一批蛋，能否孵出鸡取决于蛋质，不取决于蛋的数量；价值模型做的正是在额度决定之前识别「蛋质」；
...
**建议展示**：同额度分层双图（收入图：高价值线整体在上方；逾期率图：高价值线整体在下方），一图终结质疑。
...
- **模型定义**：新客价值模型 1.0（与创新申请材料中 New Customer Qualification Model 1.0 为同一模型），LightGBM 二分类，17 个银行流水特征，建模样本 56,330（2024-04 起），目标为「3 个月利息收入是否低于 160」的弱价值标记，目标样本占比 27.12%；
...
### P3｜做得怎么样：效果跨时间稳定，验证经得起推敲

**核心观点**：模型在跨时间样本上效果不降反升（OOT 优于训练期），分箱区分度经过「同额度分层」严格检验，具备投入经营使用的基础。

**关键信息**：

- **整体效果**：OOT AUC 0.7294 / KS 0.3467（高于 INS 的 0.7015 / 0.2939，跨时间排序稳定）；Top10% 目标率 63.00% vs Bottom 10% 目标率 7.30%（区分度 8.6 倍），Top10% 捕获 24.54% 目标样本，Lift 2.45；
...
- **如实披露的局限**：总额度 2,500+ 区间逾期维度暂无有效观察（高额度申请成交率仅约 2.6%，无放款表现样本），待样本积累后补充验证；OOT 校准存在轻微低估（约 2.15 个百分点），排序分层使用不受影响。
...
- **价值好人群**（价值 1/2 档）：工资收入总额 41,139 vs 价值差 11,130；工资入账次数 20.5 次 vs 10.6 次；Centrelink 收入占比 2.1% vs 14.6%；couple 家庭占比 43.8% vs 29.1%；行业信息完整（Missing 仅 3.5% vs 30.7%）；
- **价值差人群**（价值 4/5 档）：收入低、工资频次低、发薪周期波动大（2.72 vs 3.79）、福利收入依赖高、行业识别缺失多——更像「收入与可识别职业现金流弱」；
...
**核心观点**：价值模型不与风险模型对立，而是联合形成四象限经营策略——价值扩张以风险预算为上限，追求风险约束下的组合收益最大化。
...
- **应用原则**：额度总量受风险预算约束时，经营问题本质是「有限的额度怎么分配」；模型命题是「差异化投放优于一刀切」，可用 lift 曲线直接量化，不依赖因果结论。
...
- **增量额度试点（建议启动）**：选取一批中价值客户随机分组，实验组提高额度、对照组维持现状，3–6 个月后对比两组收入与风险表现，用实验数据回答「额度与价值」的因果问题，并量化模型投放的增量价值——需要试点额度/预算与数据口径支持【待确认：是否已启动】；
...
**核心观点**：在澳洲 Responsible Lending 监管下，Serviceability（偿付能力）计算是信审的核心；流水识别的准确性，直接决定收入、支出、负债的计算结果，进而决定每一笔授信决策的质量。

**关键信息**：

- **核心计算逻辑**：Serviceability ＝ 收入 − 必要支出 − 负债还款 ＝ 客户可支配盈余；
- **监管要求**（NCCP Act 2009 / ASIC，四项）：合理了解客户财务状况及借款目的；核实收入、必要支出、现有负债及可预见变化；评估客户能否在不造成重大经济困难的情况下履约；不得提供「不适合」的信贷产品；
...
### P2｜原有方案的四大局限：为什么必须自研

**核心观点**：原有高度依赖 illion 的分类服务存在「黑盒、粗粒度、无控制权、响应慢」四大局限，长期制约模型优化、策略调整与产品创新。
...
### P3｜自研体系整体架构：从数据到决策的完整链路

**核心观点**：已建成「IDA 发起 → IDP 调度 → BS-CAT 分类 → S-CALC 计算 → 风险模型决策 → LMS 输出」的端到端自主链路，分类模型前置产出核心结果，编排调度轻量化。

**关键信息**：

- **十步链路**：IDA 发起贷款申请与流水处理请求 → AWS SQS 消息队列 → IDP 拉取申请、预处理数据、编排流程 → 调用 BS-CAT 分类能力 → 逐笔输出交易分类、交易对手、分析标签 → S-CALC 执行收入/负债/支出与可支配盈余计算 → GenAI 辅助推理处理复杂规则与补充判断 → 结果落库 → 风险模型结合画像与历史表现输出评分/建议 → LMS 返回推荐额度、质量标签、可视化结果；
...
- **贯穿链路的支撑**：分类知识库（规则、关键词、第三方数据）+ 分类模型监控与运营（效果监控、异常跟踪、人工维护）。

**建议展示**：端到端流程图（保留可编辑 mermaid，勿用静态图片）。

### P4｜各模块之间的关系与分工

**核心观点**：各模块职责清晰、接口标准——IDA 负责发起与编排、IDP 负责数据处理与调度、BS-CAT 负责交易理解、S-CALC 负责偿付能力计算、LMS 负责决策输出；模型团队产出结果，风险/模型团队消费结果，人审闭环反馈。

**关键信息**：

- **IDA（申请发起方系统）**：发起贷款申请与流水处理请求；负责链路调用、调度逻辑、Serviceability 计算与 IDA/IDP 代码落地（对接负责人：Tien / Niral）；
...
**核心观点**：系统把「时间 + 金额 + 方向 + 交易描述」的原始流水逐笔理解，再汇总为客户级收入、支出与负债画像，直接回答「客户有多少稳定收入、承担多少必要支出、正在偿还哪些贷款、是否具备新增还款能力」。

**关键信息**：

- **交易明细维度**（每笔交易三级加工）：原始描述 → 标准交易对手 + 主体类型 → 分类与依据。示例：`BILL PAY Fair Go Finance DT.58oa72 SACC 239` → Fair Go Finance（小额信贷机构）→ SACC Loans 贷款还款；`PAYROLL ACME PTY LTD`（双周入账、金额稳定）→ ACME PTY LTD（雇主）→ 工资收入；`MCDONALDS BRISBANE` → McDonald's（商户企业）→ 消费；
- **单笔贷款画像**：借款 A$1,500、已还本金 A$936、预计剩余 4 期、月还款约 A$338、APR 42%、产品类型 SACC、状态在贷——逐笔还原真实偿债压力；
- **用户整体负债画像**：负债机构 2 家、在贷 3 笔、负债结构 SACC 65% / Non-SACC 15% / BNPL 20%、90 天借款 A$2,300、月还款/月收入 5.8%、正常还款率 89%；
- **贷款状态判断**：在贷 / 结清 / 违约中 / 状态不明，支撑「当前负债压力」与「还款中断」识别。

**建议展示**：三级加工示意——原始流水表 → 逐笔分类表 → 负债画像卡片。

### P6｜效果验证：OOT 实测已具备替代 illion 的基础

**核心观点**：35 个申请 case、44,241 条交易的 OOT 实测（2026/07/20–21）显示，自研分类在覆盖率、一致性与分类精度上达到或超过 illion，负债识别能力已可替代。
...
- **分类精度**：Credit24 的 19 笔交易全部被 illion 泛化为 SACC Loans，我方结合交易模式与机构产品知识识别为 LOC 循环授信、归入 Non-SACC（LOC 与 SACC 在监管属性、利率上限上有本质区别，误判会扭曲风控信号）；
...
**核心观点**：用「引擎校验 + 四层基线回归 + 双 Agent 运维」保证分类质量可持续迭代；9 月完成切量准备，AB 验证后替换 illion，并持续向多数据源、多场景演进。

**关键信息**：

- **质量保障机制**：每台引擎输出过 6 项校验（类型、列完整、键唯一、越权检查、类别归属等）；每次变更过四层基线回归（最终分类结果 / 引擎认领层 / 配置版本指纹 / 汇总指标层），任何规则调整可追溯、可回滚；
- **运维闭环（1 人 + 2 Agent）**：质检 Agent 监控分类漂移、命中下降、对手方异常并定位问题引擎；知识库 Agent 主动检索未知商户、交叉验证冲突知识、生成更新建议；人工确认后自动执行调整与验证——形成「监控 → 定位 → 修复 → 沉淀」闭环（对应 OKR O4-KR1 试点）；
- **上线里程碑**：8/11 负债类知识库初始化 + Liability 效果报告 + preview 部署 + 接口开放（已完成）；8/21 全类别知识库初始化 + 两个运维 Agent + 全分类效果评估报告（节点已到，交付状态【待确认】）；8/31 正式版发布 + 分类数据落库 + IDP/IDA 链路调通 + 空跑两周；9/15 新 S-CALC 完成 + LMS 页面改造 + 人审知识同步 + 新旧 IDP 链路 AB 测试；空跑验证完成后具备切量条件（下游正在评估切量时间节点）；
