...
> **状态基准日**：2026 年 8 月 27 日。基准日后的节点均为计划，不表述为已完成。

## 汇报建议｜先讲结论，再讲证据

**建议主汇报控制在 12–14 页，技术细节移至备份页。** 管理层主线建议聚焦四个问题：为什么做、已经证明了什么、9 月要落成什么、需要什么支持。

**三句话结论**：

1. 澳洲模型建设已完成风险、价值、意愿等单点能力验证，正在从“模型可用”转向“业务链路可调用、效果可衡量”；
2. 银行流水是三类模型共同的数据底座，BS-CAT 负债分类已在小样本 OOT 中达到进入并行验证的条件，但尚不能表述为已全面替代 illion；
3. 9 月的关键不是再增加模型数量，而是完成链路、落库、并行验证和切量门槛确认，让模型真正进入投放、运营与信审决策。

**建议管理层关注 / 支持事项**：

- 明确 BS-CAT 新旧链路并行验证的通过标准、观察周期、切量责任人与目标日期；
- 确认价值模型首批落地场景及增量额度试点的风险预算、样本量与观察口径；
- 明确投放、运营、风险三方的数据回传责任，保证模型效果可以形成闭环评估。

**建议页序（13 页主汇报）**：

| 页码 | 页面主题 | 本底稿对应内容 |
| --- | --- | --- |
| 1 | 执行摘要：三句话结论 + 三项支持 | 汇报建议 / 开场 |
| 2 | 为什么现在必须做分层经营 | Part 1-P1 |
| 3 | 当前阶段：单点验证完成，体系落地进行中 | 合并 Part 1-P2、P3 |
| 4 | 目标体系：风险 × 价值 × 意愿 | Part 1-P4 |
| 5 | H2 三条主线与 9 月里程碑 | 合并 Part 1-P5、P7 |
| 6 | 对风险、投放、运营的业务价值 | Part 1-P6 |
| 7 | 价值模型关键质疑：是否只在复述额度 | Part 2-P1 |
| 8 | 价值模型原理与验证结果 | 合并 Part 2-P2、P3 |
| 9 | 风险 × 价值联合策略 | Part 2-P5 |
| 10 | 价值模型试点设计与决策事项 | Part 2-P6 |
| 11 | 银行流水为何必须自主可控 | 合并 Part 3-P1、P2 |
| 12 | 目标链路与当前建设状态 | 合并 Part 3-P3、P5 |
| 13 | OOT 结果、切量门槛与管理层决策 | 合并 Part 3-P6、P7 |

**建议放入备份页**：价值模型人群画像（Part 2-P4）、BS-CAT 模块分工（Part 3-P4）、模型全量卡片、典型交易案例和详细 OKR 映射。这样主汇报保留结论与决策，追问时仍有证据可展开。
...
| 主题 | 资料 | 主要用途 |
| --- | --- | --- |
| 投放协同 | `res/投放团队 × 模型团队 2026H2 项目合作清单.md` | 投放痛点、P0/P1 项目、归因与回传规划 |
| 运营协同 | `res/运营团队 × 模型团队 2026H2 项目合作清单.md` | LTV、三维分层、定价和运营 Agent 规划 |
| 价值模型 | `res/价值模型汇报材料.MD` | 模型定义、样本、AUC / KS、分箱与画像 |
| 额度与价值 | `res/价值模型专项汇报_额度与价值关系说明.MD` | 同额度分层验证、模型能力边界与试点思路 |
| BS-CAT 架构 | `res/银行流水分类模型架构说明.md` | 9 引擎、编排逻辑、输出契约与质量保障 |
| BS-CAT 效果 / 进度 | Liability 效果报告、项目进展材料【源文件待补充】 | 35 个 case OOT、8–9 月里程碑 |
| 业务目标 | 2026H2 OKR / 公司 O2-KR1【源文件待补充】 | NON SAAC、风险与项目优先级目标 |
| 澳洲监管 | [ASIC Responsible lending](https://asic.gov.au/regulatory-resources/credit/responsible-lending/)；[National Consumer Credit Protection Act 2009](https://www.legislation.gov.au/C2009A00134/latest/text) | 合理询问 / 核实、不适合性评估、SACC 90 天账户交易要求 |

### 上屏前必须确认

1. “风险表现 7.1%”的指标定义、账龄、样本范围和观察窗口；
2. 8/21 三项交付物（全类别知识库、两个运维 Agent、全分类效果报告）的实际完成状态；
3. Serviceability 计算在 IDA 与 S-CALC 之间的最终职责边界；
4. 老客风险模型 V2 的监管确认状态与预计替换日期；
5. 价值模型各业务场景的真实上线状态，以及增量额度试点是否已获批 / 启动；
6. BS-CAT 并行验证的样本量、关键类别准确率 / 召回率、系统稳定性阈值和切量负责人；
7. 价值模型高额度区间的有效表现样本是否已有新增，概率校准是否已完成；
8. 召回“13.6k → 约 30 Leads”的时间范围、渠道、去重方式和对照基准。

### 统一口径

- **已上线**：生产环境已有真实流量，并有监控与回退机制；
- **已部署 / preview**：能力已部署或接口已开放，但不等于已承接生产决策；
- **已验证**：完成离线、OOT 或独立样本评估，不等于产生线上增量价值；
- **具备切量条件**：链路、数据、效果、稳定性和回退机制均达到预设门槛；
- **计划**：基准日后尚未验收的节点，统一写为“计划完成”，不写“完成”。
